...
function ComplaintForm() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        setCustomerName(response.data.user.name); // ✅ Set customer name
        setCustomerID(response.data.user._id);   // ✅ Set customer ID
        setLoading(false);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(complaintData);  // Log the complaint data to the console before sending
