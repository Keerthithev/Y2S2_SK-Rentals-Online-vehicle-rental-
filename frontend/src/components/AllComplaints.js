...
const AllComplaints = () => {
  ...
  const handleReply = async (complaintID) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // SweetAlert error
        Swal.fire('Error', 'Failed to submit reply. Please try again.', 'error');
      }
    } catch (error) {
      ...
      // SweetAlert error
      Swal.fire('Error', 'There was an issue submitting your reply.', 'error');
    }
  };
  ...
  const exportToExcel = async () => {
    ...
    const isConfirmed = await Swal.fire({
      ...
      text: 'Do you want to export all complaints to Excel?',
      icon: 'question',
      showCancelButton: true,
