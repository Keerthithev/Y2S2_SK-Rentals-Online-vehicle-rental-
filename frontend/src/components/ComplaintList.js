...
function ComplaintList() {
  ...
  const handleDelete = async (complaintID) => {
    ...
    if (!isConfirmed.isConfirmed) return; // If the user clicks "Cancel", return early
  ...
  const handleEdit = (complaint) => {
    Swal.fire({
      title: 'Do you want to edit this complaint?',
      icon: 'question',
      showCancelButton: true,
      confirmButtonText: 'Edit',
      cancelButtonText: 'Cancel'
    }).then((result) => {
      if (result.isConfirmed && complaint.status !== 'Resolved') {
